...
这里的是**业务领域页面样板**（"成品长什么样"——真实标杆页面的结构、列、操作与规范注解）；kit 内置的 9 个 `TPL-*.md` 是**代码生成模板**（"骨架怎么生成"）。用法：把本库模板作为 [page-codegen](/frontend/pc/skills/page-codegen) 的参照输入，AI 对照生成，产出过 [规范审计](/frontend/pc/skills/convention-audit) 验收。
