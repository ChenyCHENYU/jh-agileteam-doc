...
点击列权限可以设置接口过滤掉某些列的数据。在操作栏点击列权限按钮进入配置页面，勾选需要过滤（隐藏）的列字段后保存，该接口返回数据时将不再包含所选列；支持对已配置的列权限进行编辑或删除。
...
点击行权限可以设置接口只返回符合某些规则的数据。在操作栏点击行权限按钮进入配置页面，通过设置字段条件（如等于、不等于、包含、大于、小于等）及逻辑关系限定返回的数据范围，仅返回符合规则的数据行；支持对已配置的行权限进行编辑或删除。
...
| 标签 | 释义 |
| --- | --- |
| 客户端ID | 客户端的唯一标识，用于在授权流程中区分不同的客户端应用。 |
| 客户端密钥 | 客户端的访问密钥，用于校验客户端身份，保障接口调用的安全性。 |
| 客户端权限集 | 能否访问的系统和菜单的授权。 |
| 授权模式 | 选择不同的授权模式进行客户端授权。 |
| 令牌时效 | 设定登录之后有效访问时间，就是距离登录到登录失效的时间。 |
| 回调地址 | 登录授权失败之后的回调地址。 |
