...
::: tip ✅ 已正式发布
后端 Skills 包（`@agile-team/wl-skills-bd`，当前 **v0.18.1**）已正式发布，覆盖框架扩展点 Bean 与容器测试闭环、生产安全契约、通用契约与运行时边界闭环、契约驱动代码生成、模块目录与精准上下文、配置分层与多环境、任务驱动、数据安全护栏、行为契约测试、质量门、MCP 与安全修复闭环全链路。
:::
