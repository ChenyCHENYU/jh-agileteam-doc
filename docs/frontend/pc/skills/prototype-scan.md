...
AI：[Pre-flight]
    ...
    └─ 不会生成代码
    完成，页面清单：
    1. mmwr-customer-archive (LIST) - 客户档案查询
...
| 页面命名不符合 kebab-case | AI 直接用中文/驼峰命名 | 指令里强调"命名遵循 standards/page.md" |
| 同一字段在多页面命名不一致 | AI 没有跨页面对齐 | 要求生成清单后做一遍"字段一致性自检" |
| LIST/MASTER_DETAIL 类型错判 | 原型隐含主从结构未识别 | 显式告知"这是主从表"或贴出原型图 |

## FAQ

**Q：扫描出来的页面清单可以直接 page-codegen 吗？**  
A：建议先走 `api-contract` 生成 api.md，让后端确认字段，再 codegen。否则 data.ts 里的字段名可能和后端对不上。

**Q：能不能跳过这个 Skill 直接让 AI 写代码？**  
A：可以但不推荐。Skill 的价值是约束输出物落到统一文档，便于团队对账。

**Q：扫描结果不对怎么办？**  
A：reports/ 下的 md 可直接编辑。修改后再让 AI "基于 reports/PROTOTYPE_SCAN_xxx.md 进行 api-contract"。
