...
`@agile-team/wl-skills-kit`（v2.11.5）提供 11 个 CLI 子命令，覆盖安装、升级、清理、校验、修复、体检、导出全生命周期。所有命令默认作用于当前工作目录，均可加 `--dry-run` 预览。

## 命令速查

| 命令 | 用途 |
|------|------|
| `init`（默认） | 全量安装（14 规范 + 11 Skill + 17 MCP + 10 编辑器配置 + 组件文档） |
