...
`@agile-team/wl-skills-kit`（v2.16.9）提供 14 个 CLI 子命令，覆盖安装、升级、清理、校验、修复、体检、导出、契约、组件治理全生命周期。所有命令默认作用于当前工作目录，均可加 `--dry-run` 预览。
...
| `validate` | 静态校验页面完整性 + AST 语义级 R1~R19（CI 卡门） |
...
静态校验页面文件完整性（4 文件、AGGrid、cid、mock、api.md），含 AST 语义级 R1~R19 检测。R13 圈复杂度、R15 分页边界、R16 运行时边界默认执行；R14 类型错误需 `--typecheck` 开启；R17 表单仅必填开关按实际绑定逐项判断；R18 表单校验库版本检查；R19 弹窗内 AG Grid 必须用 `v-if` 延迟挂载（防零高度渲染）。
