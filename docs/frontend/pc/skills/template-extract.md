...
`提取模板` / `抽取模板` / `沉淀模板` / `模板贡献` / `template-extract` / `封装模板`
...
🔍 识别结果
  ...
  - 结论建议：本页面已有相似模板，可能是 TPL-OPERATION-STATION 的变体，
              建议考虑增强现有模板而非新建。
              （或：本页面是独立新模式，建议提取为 TPL-XXX）
```
...
| Skill | 关系 |
|-------|------|
| **convention-audit** | 目标页必须先通过审计（合规）才可提取 |
| **page-codegen** | 提取后的模板会被 page-codegen 引用 |
| **code-fix** | 如目标页有偏差，建议先 code-fix 整改再提取 |
