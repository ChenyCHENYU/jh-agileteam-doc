...
> 📦 来源：`wl-mbase` 仓库 `docs/移动端消息中心使用与架构说明.md`（2026-09-16 版，含审批详情分层与地址使用规范）。

<AuthorTag :authors="['CHENY']" />

> **声明与致谢：** 移动端消息中心的统一接入、架构梳理和文档完善，离不开平台组小伙伴在平台能力、接口联调和技术经验方面给予的支持。特别感谢大家开放协作、倾囊相助的高风亮节，谨致敬意。
...
#### 7.3.1 是否必须进入子应用查看业务数据

不必须。移动审批采用“基座承载通用审批，子应用承载领域业务”的分层方式，应按业务复杂度选择：

| 场景                                                     | 推荐呈现方式                               | 是否需要 `mobileBusinessUrl` |
| -------------------------------------------------------- | ------------------------------------------ | ---------------------------- |
| 只需流程名称、申请人、时间、审批路线和审批操作           | 全部由基座审批详情呈现                     | 否                           |
| 还需查看少量、稳定的业务摘要，例如金额、供应商、单据编号 | 将摘要写入流程 `params`，直接在基座呈现    | 否                           |
| 包含明细表、附件、图片、复杂表单或需要查看实时业务状态   | 基座呈现摘要，完整内容进入子应用只读详情页 | 是                           |
| 需要继续办理领域业务，而不仅是通过、驳回等流程动作       | 进入子应用业务页面                         | 是                           |

因此，“业务页面”是按需提供的增强入口，不是审批详情成立的前提。简单审批应尽量在基座内完成；复杂审批不应把整张业务表、明细列表或附件内容复制进流程参数。

当前分层是推荐实践，原因如下：

- 基座只依赖流程标准契约，可以稳定服务不同业务域，不需要逐个调用环保、安全、设备等领域接口。
- 流程参数是发起时的审批摘要和快照；子应用页面可以展示完整、实时的业务数据，两者用途不同。
- 领域接口、字段、附件和权限变化由对应子应用独立发布，不会迫使基座跟随每个业务版本修改。
- 子应用负责校验当前用户是否为流程参与人或具备业务查看权限，基座不能因为拿到业务 ID 就绕过领域权限。

不推荐让基座根据模板编码、流程名称或业务类型硬编码领域接口，也不推荐把 PC 组件名、低代码组件标识或 `lowcode?...` 当作移动业务地址。若未来确需在基座内呈现更丰富的通用内容，应另行定义受控、版本化的结构化详情协议，而不是允许业务注入任意组件。

#### 7.3.2 三类地址和数据不要混用

FLOW 消息从消息中心到业务单据共分三层：

```text
OAMS 审批消息 showUrl
  -> 基座解析 instanceId/commentId，进入内置审批详情
  -> 流程接口返回 paramTemplate + params，呈现审批业务摘要
  -> flowInstance.mobileBusinessUrl，按需进入子应用完整业务页面
```
...
需要在基座中直接展示的每个摘要字段，都必须同时满足：

1. 流程模板“参数配置”中存在稳定且唯一的参数编码。
2. 发起流程时，`params` 中存在完全相同的 `code` 并带有实际值；编码区分大小写。
3. 复杂对象、明细数组和附件不直接塞入 `params`，改由移动业务页面查询。
4. 若模板中配置了 `businessId` 并希望页面显示该行，除实例级 `businessId` 外，仍应在 `params` 中传入同名值。

推荐的流程发起数据如下；字段名以实际流程服务契约为准：

```json
{
  "templateSn": "BUSINESS_APPROVAL",
  "businessId": "123456",
  "businessType": 1,
  "flowName": "业务审批",
  "businessUrl": "PC端原有详情地址",
  "mobileBusinessUrl": "/mbase/example/order/detail?id=123456",
  "params": [
    { "code": "businessId", "value": "123456" },
    { "code": "orderNo", "value": "ORDER-20260914001" },
    { "code": "amount", "value": "1000" },
    { "code": "applicantDept", "value": "生产部" }
  ]
}
```

业务系统如果只向自己的 `startFlow` 接口传业务 ID，则该业务服务端必须查询业务记录，并负责组装上述流程实例字段和 `params`；基座不会反向调用业务接口补齐这些数据。

`mobileBusinessUrl` 只有满足以下条件才有效：

- 对应子应用已经实现可直接访问的移动端详情路由，并能根据业务 ID 加载指定单据。
- 使用同源 `http/https` 地址，或以 `/` 开头的相对地址。
- URL 路径命中 `src/config/portal-apps.ts` 中已启用应用的 `mpPath`。
- 流程服务已将该字段正确持久化，并由审批详情接口原样返回。
- 子应用后端使用 token 和公司上下文校验访问权限；无业务菜单权限但属于流程参与人的场景，应有明确的只读授权策略。

流程模板或发起逻辑调整后只自然影响新流程。历史实例缺少的 `params/mobileBusinessUrl` 不会自动补齐，应使用新发起实例验收；如必须支持历史记录，由流程或业务服务端制定受控的数据补齐方案。
...
flowInstance.businessUrl
flowInstance.mobileBusinessUrl
paramTemplate
params
```
...
2. 保留前一天的过期 token 冷启动，首次点击工作通知仍在免登后直达本次待办，不先落工作台，也不需要点击第二次。
3. 已登录且 token 有效时冷启动，仍消费本次 `redirect_url`，不先落工作台。
4. 弱网或免登失败时给明确提示，可重新进入。
5. 普通地址跨域或不在 `APP_PATHS` 时被拦截。
6. `FLOW_COMMENTS` 缺 `id/commentId` 时显示模板配置错误。
7. 仅对允许并启用个人通道的项目，验证钉钉工作通知接收人与内部账号映射；华新客户跳过此项。
8. token 失效时服务端刷新并重试一次。
9. 钉钉送达但消息中心不存在时，检查模板是否漏选 `pc`。
