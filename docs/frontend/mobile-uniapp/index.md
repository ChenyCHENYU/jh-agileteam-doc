...
`wl-mbase`（华新移动端门户基座，当前 **v1.0.6**）是基于 **Vue 3 + UniApp + TypeScript** 的企业移动端**统一入口**——不是业务应用，而是承载所有业务子应用的基础平台。一套源码通过 UniApp 条件编译，同时运行于**微信小程序 / 钉钉 H5 / 普通浏览器 H5 / App（含 PDA）** 四端，提供统一登录、子应用免登、公司上下文、跨宿主能力桥和多端安全构建。

> **近期增强（2026-09）**：基座请求上下文隔离（子应用间请求互不串扰）、子应用页面状态协议与质量门禁（含构建体积预算 `check-bundle-budget`）、非模板 H5 工程环境统管（非 Robot_H5 模板项目也可纳入统一环境管理）、消息中心审批详情分层与地址使用规范、构建产物统一中国时区与按分支自动构建（main 为生产主源）。
...
| App / PDA     | UniApp 原生容器        | WebView + token + `mbase_host=app` 宿主标记 | App 原生桥接（`uni.postMessage` + `evalJS` 回传） | 基座自定义 |

> 同一套源码通过 UniApp 条件编译（`#ifdef MP-WEIXIN` / `#ifdef H5` / `#ifdef APP-PLUS`）+ 运行时 UA 检测（`isDingTalkEnv`）自动适配各端差异。
