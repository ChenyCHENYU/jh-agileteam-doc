...
场景：`page-codegen` 产出了客户档案页面，上线前要挂**菜单、字典、权限**三处后台——过去这是最容易漏、最繁琐的一步。

**没有 MCP 时**：切 3 个后台界面人工点（菜单管理建目录/菜单 → 字典管理录枚举 → 权限管理配角色+动作），≥ 15 分钟，漏一处就是联调事故。

**有 MCP 后**：对 AI 说"同步菜单/字典/权限" → `menu-sync` 自动 `wls_menu_query` 查现有菜单树 → 与 page-spec 对比 → `wls_menu_upsert` 补差异 → `permission-sync` 完成角色 + 授权 + 动作闭环。

| 步骤 | 过去（手工） | 现在（MCP 驱动） |
|------|------------|----------------|
| 菜单 | 10 次手动操作 / 20 分钟 | **1 分钟 / 0 次手动** |
| 权限 | 切 3 个界面 ≥ 15 分钟 | **1 分钟 / 0 次手动** |
| Token | — | 节省约 **87%** |

写操作全部经过 `_mcp-guardrail.md` 护栏（先查后写、幂等去重），不是裸调接口。
