...
| 上下文压缩 | `standards/index.md` 任务类型 → 规范子集映射，按需懒加载（避免一次性塞入 14 条规范） |
...
直接把 14 条规范全部载入 = 每次 token 浪费 ~8000 tokens，且 AI 注意力分散。
...
| 规范审计 | 全量 14 条（convention-audit 专用） |
| 样式修改 | 仅 SCSS 相关 |

## 场景实践：同一个需求的两种问法

场景：让 AI 写一个"客户档案管理"列表页（Vant 风格的 PC LIST 页，含分页与字典列）。

**❌ Before — 直接描述需求：**

```text
帮我用 Vue3 写一个客户档案管理页面，要能查询和分页
```

AI 自由发挥的典型产出：el-table 原生组件、无 cid、无 api.md、Mock 缺失、命名随意——功能是对的，回到项目规范要改 6~7 处，改完才算"开始 Review"。

**✅ After — 注入规范上下文（kit 安装后自动生效）：**

```text
生成客户档案列表页。字段：客户编码 / 客户名称 / 等级（字典 crm_level）/ 跟进人。
搜索：名称模糊 + 等级下拉。
```

AI 的行为变化：`copilot-instructions.md` 已在系统级载入 → 匹配 page-codegen 触发词 → Pre-flight 声明"已读取 02/04/06/12 规范 + TPL-LIST 模板" → 产出 BaseTable + render-type="agGrid" + cid + api.md + mock 的标准 4 文件。

**效果**：违规率 < 5%，单页 45 分钟 → 约 8 分钟（见[总览页效果量化](./index#效果量化-前端场景实测)）。差别的来源不是提示词写得更花，而是**项目上下文已经在场**——这正是 L1 全部设计的目的。
