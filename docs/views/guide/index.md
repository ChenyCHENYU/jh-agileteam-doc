...
| 日期 | 更新 |
|------|------|
| 2026-09-17 | 同步 kit v2.21.0（引擎懒加载/K18 读盘修复/数量门禁）与 bd v0.26.0（capabilities 能力清单、状态机闭环校验、db review/snapshot-template，MCP 17→18）；mbase 基座近期增强（请求上下文隔离/页面状态协议/非模板 H5 统管）入站 |
| 2026-09-16 | **工程生态板块上线**（14 包 npm 数据看板 + MachTable/工具链/基础库）；导航重组（死链清除、九入口）+ 首页真数字改造；wl-skills-test **v0.25.0 npm 发布**并全站同步；团队页填充、AI 工作流重设计落地、模板库/样式方案补全、sidebar 校验脚本入库挂构建钩子 |
| 2026-09-10 | 同步 kit v2.20.4（validate 双类新阻断）/ test v0.24.0（产物 JSON 化 + 测试计划数据侧）；MES 表检放行码回传接口入站 |
| 2026-09-03 | 同步 test v0.21~0.22（AI 接入故事：setup + gen-contract + test-onboarding）；脚手架板块优化（版本说明页 / 外部 Catalog 指南 / Node ≥22.12 口径统一） |
...
五包之外，团队在 **@agile-team** scope 下还维护 **MachTable 企业数据表格**（16 万月下载、28 个外部依赖方）、五编辑器标准化配置、多栈脚手架等 **14 个 npm 包**——完整索引与选型见 [工程生态](/views/ecosystem/)。
