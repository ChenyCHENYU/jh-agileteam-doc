...
## 后端 Skills 集合（v0.17.9）

`@agile-team/wl-skills-bd`：16 个 MCP 工具 + 12 个 Skill，覆盖生产安全契约、通用契约与运行时边界闭环、契约驱动代码生成、模块目录与精准上下文、配置分层与多环境、任务驱动、数据安全护栏、行为契约测试全链路，与前端 Skills 三包协作。
